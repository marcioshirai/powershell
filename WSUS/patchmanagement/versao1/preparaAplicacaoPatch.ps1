...
# se ja existir gmuds no acompanhamento, carregar e adicionar com as preparadas agora
if (Test-Path "$($acompanhamentoFolder)\indexGmuds.csv") { 
    $antigo +=  Import-Csv "$($acompanhamentoFolder)\indexGmuds.csv" -Delimiter ";" 
    if ($antigo) { $filesExecutar += $antigo }
    }
...
$filesExecutar | Export-Csv "$($acompanhamentoFolder)\indexGmuds.csv" -Delimiter ";" -NoTypeInformation -Force 
